...
mod tests {
    ...
    #[test]
    fn load_soul_supports_tilde_path() {
        ...
        let soul_path = home.join(&file_name);
        std::fs::write(&soul_path, "TEST-SOUL-TILDE").expect("write soul");

        let mut config = Config::default();
        config.core.soul_file = Some(format!("~/{}", file_name));

        let loaded = MasixRuntime::load_soul(&config).expect("soul should load");
        assert!(loaded.contains("TEST-SOUL-TILDE"));

        std::fs::remove_file(&soul_path).ok();
    }

    #[test]
    fn load_soul_supports_relative_path_against_data_dir() {
        let temp_dir = std::env::temp_dir().join(format!(
            "masix-test-soul-rel-{}",
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
        ...
        let soul_path = temp_dir.join("SOUL.md");
        std::fs::write(&soul_path, "TEST-SOUL-REL").expect("write soul");

        let mut config = Config::default();
        config.core.data_dir = Some(temp_dir.to_string_lossy().to_string());
        config.core.soul_file = Some("SOUL.md".to_string());

        let loaded = MasixRuntime::load_soul(&config).expect("soul should load");
        assert!(loaded.contains("TEST-SOUL-REL"));
...
impl MasixRuntime {
    ...
    fn load_soul(config: &Config) -> Option<String> {
        ...
        let base_data_dir = if let Some(data_dir) = &config.core.data_dir {
            ...
            dirs::home_dir()?.join(".masix")
        };

        let resolved_path = Self::resolve_path_with_base(soul_path, &base_data_dir).ok()?;

        if let Ok(content) = std::fs::read_to_string(&resolved_path) {
            info!("Loaded SOUL.md from {}", resolved_path.display());
            Some(format!(
                "Sei un assistente AI. Ecco la tua identità e valori:\n\n{}\n\nRispondi in italiano quando possibile.",
                content
            ))
        } else {
            warn!("Failed to load SOUL.md from {}", resolved_path.display());
            None
        }
    }
    ...
    async fn execute_tool_call(
        ...
        // Server names may contain underscores (e.g. "codex-tools", "discovery"),
        // so we resolve the server by longest prefix match instead of splitn.
        if mcp_client.is_none() {
            return Err(anyhow::anyhow!(
                "No MCP client available for tool: {}",
                tool_name
            ));
        }
        ...
        let mcp_tool_name = mcp_tool_name.as_str();
