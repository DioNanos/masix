...
async fn manage_linux_boot_autostart(
    ...
    match action {
        BootAction::Enable => {
            ...
            if let Some(status) = try_enable_linux_system_service(&stable_bin, config_path, home).await?
            {
                return Ok(status);
            }

            if let Some(status) = try_enable_linux_user_service(&stable_bin, config_path, home).await?
            {
                return Ok(status);
            }
        ...
        BootAction::Disable => {
            ...
            let _ = run_status_command("systemctl", &["disable", "--now", LINUX_SYSTEMD_SERVICE_NAME]);
...
async fn manage_macos_boot_autostart(
    ...
    match action {
        BootAction::Enable => {
            ...
            if let Some(parent) = system_plist.parent() {
                if tokio::fs::create_dir_all(parent).await.is_ok() {
                    ...
                    if tokio::fs::write(&system_plist, plist).await.is_ok() {
                        ...
                        let _ = run_status_command("launchctl", &["bootstrap", "system", MACOS_LAUNCH_DAEMON_PATH]);
                        let _ = run_status_command(
                            "launchctl",
                            &[
                                "enable",
            ...
            let _ = run_status_command(
                "launchctl",
                &["bootstrap", &format!("gui/{}", uid), user_plist.to_string_lossy().as_ref()],
            );
...
fn render_macos_launch_plist(
    ...
    let mut args = vec![
        format!("<string>{}</string>", xml_escape(&masix_bin.display().to_string())),
